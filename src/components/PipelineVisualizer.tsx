...
export default function PipelineVisualizer({ state }: PipelineVisualizerProps) {
  ...
  const handleDownload = () => {
    ...
    link.href = '#';
    link.download = \`peta_transmutation_\${state.targetLang}.mp3\`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    alert(\`Starting download of transmuted track (\${state.targetLang.toUpperCase()} Vector)\`);
  };
      ...
      {state.outputReady && (
        <div className="mt-6 p-4 rounded-lg border border-emerald-500/20 bg-emerald-500/5 text-center">
          ...
          <button 
            ...
            className="mt-3 px-4 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-medium hover:bg-emerald-500/20 transition-all active:scale-95"
          >
            Download Output
          </button>
        </div>
